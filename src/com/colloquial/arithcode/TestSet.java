...
/**
 * Package local helper class to compute statistics for a set of compression
 * experiments. Stores original and coded bytes, average and cumulative
 * compression and speeds. Pretty printing is through report.
 * 
...
class TestSet {
	...
	/**
	 * Records an outcome for a compression for a given test name, specifying
	 * original and coded bytes, along with encode and decode times.
	 * 
	 * @param name
	 *            Name of test.
	 * @param originalBytes
	 ...
	 * @param encodeTime
	 *            Number of milliseconds required to encode.
	 * @param decodeTime
	 *            Number of milliseconds required to decode.
	 */
	void record(String name, int originalBytes, int codedBytes, long encodeTime, long decodeTime) {
		if (!_tests.containsKey(name)) {
			_tests.put(name, new TestStatistics(name));
		}
	...
	public String toString() {
		...
		while (it.hasNext()) {
			results.add(_tests.get(it.next()));
		}
