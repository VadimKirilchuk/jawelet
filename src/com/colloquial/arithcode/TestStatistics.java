...
/**
 * Package local helper class to compute statistics for a single compression
 * experiment. Stores original and coded bytes, average and cumulative
 * compression and speeds. Pretty printing is through toString.
 * 
...
class TestStatistics implements Comparable {
	...
	public String toString() {
		...
		report.append("\n");
		report.append("Overall Compression Rate " + roundOff(8.0 * rate(_totalCodedBytes, _totalOriginalBytes))
				+ " b/B");
		report.append("\n");
	...
	/**
	 * Record the result of an experiment with specified number of original and
	 * encoded bytes with specified encode and decode time.
	 * 
	 * @param originalBytes
	 *            Number of bytes in original source.
	 * @param codedBytes
	 *            Number of bytes in decoded result.
	 * @param encodeTime
	 *            Time taken to do encoding in milliseconds.
	 * @param decodeTime
	 *            Time taken to do decoding in milliseconds.
	 */
	 ...
	 * 
	 * @return The string representation of the statistics for this test on one
	 *         line.
	 */
	String lineReport() {
		return (_name + roundOff(8.0 * rate(_totalCodedBytes, _totalOriginalBytes), 8, 2)
				...
				+ roundOff(_totalEncodeSpeed / (double) _numTests, 8, 0) + roundOff(_totalDecodeSpeed
				/ (double) _numTests, 8, 0));
	}

	/**
	 * Compare using average compression rates, breaking ties by name. Assumes
	 * tests have been run on same set.
	 */
	private int compareTo(TestStatistics that) {
		if (_totalCompressionRate < that._totalCompressionRate)
			return 1;
		if (_totalCompressionRate > that._totalCompressionRate)
			return -1;
	...
	/**
	 * Round off a double to specified number of decimal places, taking up
	 * specified total width
	 * 
	 * @param x
	 *            Double to round off and convert to string.
	 * @param width
	 *            Width of final string representation.
	...
	/**
	 * Round off a double with a multiplicative factor, padding to specified
	 * width.
	 * 
	 * @param x
	 *            Double to round off and convert to string.
